...
var server = http.createServer(function (request, response) {
	...
	case "DELETE":
                var msgId = (url.path).substring((url.path).lastIndexOf('/') + 1, (url.path).length);
	        if ((url.path).includes("/messages/"))
	        {
		        if (isNaN(msgId)) //not a number
			{
		...
		else if ((url.path) ==="/stats" || (url.path) ==="/messages" || (url.path).includes("/messages?"))
                {
	...
	case "POST":
                if ((url.path) === ("/stats") || (url.path).includes("/messages/") || (url.path).includes("/messages?"))
		{
			response.writeHead(405);
			response.end();	
		}
		else if ((url.path) ==="/messages")
		{
			...
			request.on('data', function (chunk) {
		        ...
		        request.on('end', function () {
                                ...
                                if (data.name !== "Anonymous" && data.name !== undefined)
                                        data.image = MD5(data.email);

                                if (data.name == "Anonymous" && data.name !== undefined)
                                        data.image = "images/ann.png";
	...
	case "GET":
            if ((url.path).includes("/register?usermail=")) // add new user
            { 
                email = ((url.path).substring((url.path).indexOf('=') + 1, (url.path).length));
                //name = name.replace(/%20/g, ' '); //replace ' ' into '/%20/g' 
            ...
            else if ((url.path).includes("/logout?usermail=")) //remove user
            {
                email = ((url.path).substring((url.path).indexOf('=') + 1, (url.path).length));
                //name = name.replace(/%20/g, ' ');
                if (Babble.usersArr.includes(email)) 
                {
	...
	default:
		response.writeHead(400); //not a valid method... it says its a bad request? or 405?
                response.end();
	};
}); 

// GRAVATER MD5 function
